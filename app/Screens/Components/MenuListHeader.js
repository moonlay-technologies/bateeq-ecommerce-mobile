...
const MenuListHeader = ({menuItems, dataStory}) => {
  const [activeSubMenu, setActiveSubMenu] = useState(null);
  const navigation = useNavigation();

  const toggleSubMenu = menu => {
    setActiveSubMenu(prevActiveMenu =>
      prevActiveMenu === menu.id ? null : menu.id,
    );
    if (menu.title === 'OUR STORY') {
      navigation.navigate('PageOurStory', {pageStory: dataStory});
    }
  };
    ...
    return dataMenu?.menu?.items?.map(item => (
      <View key={item?.id} style={styles.menuItem}>
        ...
        {item.items && activeSubMenu === item.id && (
          <View
            style={item.items.length !== 0 ? styles.subMenuContainer : null}>
  ...
  return (
    <View style={{flexDirection: 'row'}}>{renderMainMenu(menuItems)}</View>
  );
};
