...
function Home(props) {
  ...
  const handleLike = id => {
    const temp = productsData.map((data, index) => {
      if (id === data.id) {
  ...
  const handleLike2 = id => {
    const temp = productsData2.map((data, index) => {
      if (id === data.id) {
        return { ...data, isLike: !data.isLike };
      }
      return data;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.backgroundColor }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 8, marginBottom: 15 }}>
