          ...
          <View style={{ flexDirection: 'row', justifyContent: 'flex-start' }}>
            ...
            {props?.lineItems && Array.isArray(props?.lineItems?.nodes) && props?.lineItems?.nodes.length > 0 ? (
              <View style={{ flexDirection: 'row', width: 100 }}>
                {props?.lineItems?.nodes?.slice(0, 2).map((item, index) => (
                  ...
                  >
                    {item?.product?.images &&
                      item?.product?.images?.nodes &&
                      Array.isArray(item?.product?.images?.nodes) &&
            ...
            <View>
              <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 2 }}>
                {['Order', props?.name].join(' : ') ?? '-'}
              </Text>
              <Text style={{ fontSize: 16, fontWeight: 'bold' }}>
                {['Rp', formatWithCommas(props?.totalPriceSet?.presentmentMoney?.amount ?? 0)].join('. ') ?? '-'}
              </Text>
              <View style={{ flexDirection: 'row', marginVertical: 4 }}>
                <Text style={{ fontSize: 13 }}>{'Total Quantity : '}</Text>
                <Text style={{ fontSize: 13, fontWeight: 'bold' }}>
                  {findKey(props, ['subtotalLineItemsQuantity']) ?? '-'}
                </Text>
              </View>
              <View
                style={{
