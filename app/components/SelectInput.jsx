...
function SelectInput({ label, options, onSelect, placeholder, customDetail, errors = {}, name, selectedValue }) {
  ...
  return (
    <View style={styles.inputGroup}>
      ...
      >
        <Text style={styles.selectText}>{selectedOption ? selectedOption.label : placeholder || 'Select'}</Text>
        <AntDesignIcon name="caretdown" size={16} color={customDetail ? COLORS.title : COLORS.text} />
      </TouchableOpacity>
      {isDropdownOpen && (
        <View style={styles.optionsContainer}>
          {options?.map((option, idx) => (
            <TouchableOpacity
              ...
              style={[styles.option, selectedOption && option.value === selectedOption.value && styles.selectedOption]}
