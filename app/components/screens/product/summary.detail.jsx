     ...
     <View style={{flex:1,flexDirection:"row",justifyContent:"space-between",width:"100%"}}>

       {
         typeof(price?.discount) === 'boolean' ?
           <Text style={{fontWeight:"700",fontSize:18,color:"#000000"}}>{[price?.prefix, formatWithCommas(Number(price?.regular))].join('. ') || ''}</Text>
         :
           price?.discount && price?.discount > price?.regular ?(
             <Text style={{fontWeight:"700",fontSize:18,color:"#000000"}}>{[price?.prefix, formatWithCommas(Number(price?.regular))].join('. ') || ''}</Text>
           ): (
             <Text style={{fontWeight:"700",fontSize:18,color:"#000000"}}>{[price?.prefix, formatWithCommas(Number(price?.discount))].join('. ') || ''}</Text>
           )
       }
     ...
     </View>
     {
       price?.discount && (
         <View style={{
           ...
           <Text style={{textDecorationLine:"line-through"}}>{typeof(price?.regular) !== 'undefined' && price?.regular ? [price?.prefix,price?.discount].join('. ') : "-"}</Text>
         </View>
       )
     }
     
     <View style={{marginTop:5}}>
       <Text numberOfLines={3} style={{fontWeight:"bold",fontSize:15,color:COLORS.title}}>{title}</Text>
     </View>
   </View>
  );
}
