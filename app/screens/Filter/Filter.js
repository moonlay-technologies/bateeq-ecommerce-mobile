...
function Filter() {
  ...
  const handleFilterSelected = val => {
    const Gender = genderFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Discount = discountFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Size = sizeFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Price = priceFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const CustomerRating = customerRatingFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Type = typeFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Occasion = occasionFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Brand = brandFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Color = colorFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Sleeve = sleeveFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Fabric = fabricFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Offers = offersFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Theme = themeFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Reversible = reversibleFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Pattern = patternFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
    });
    const Availability = availabilityFilter.map(data => {
      if (val === data.title) {
        return { ...data, selected: !data.selected };
      }
      return data;
          ...
          <ScrollView>
            {filterData.map((data, index) => (
              <List.Item
                ...
                title={() => (
                  <Text style={{ ...FONTS.font, ...FONTS.fontMedium, top: -1, color: COLORS.title }}>{data.title}</Text>
                )}
              />
            ))}
          </ScrollView>
