...
import { IterableAttributionInfo, IterableCommerceItem, IterableActionContext, EventName, IterableAction, IterableActionSource } from '../Iterable';
...
test('urlHandler_canOpenUrlSetToTrueAndUrlHandlerReturnsFalse_openUrlCalled', async () => {
    ...
    const config = new IterableConfig();
    config.urlHandler = jest.fn((url, _) => {
        return false;
    });
...
test('customActionHandler_actionNameAndActionData_customActionHandlerCalled', () => {
    ...
    const config = new IterableConfig();
    config.customActionHandler = jest.fn((action, context) => {
        return true;
    });
    // initialize Iterable object
    // eslint-disable-next-line @typescript-eslint/no-floating-promises
